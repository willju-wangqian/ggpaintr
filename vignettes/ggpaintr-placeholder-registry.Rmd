...
`ggpaintr_formula()`, `ggpaintr_app()`, `ggpaintr_server_state()`, or
...
- `context`: a named list containing `ggpaintr_obj`, `placeholders`,
...
```{r custom-copy}
date_copy_rules <- ggpaintr_effective_copy_rules(
  list(
...
> outside the `ggpaintr_placeholder()` call can also be serialized.
