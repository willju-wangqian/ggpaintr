...
- **`ui_text`** — override labels, help text, placeholder hints, and
  the fallback text for empty inputs. See §3e.
...
`ids = ...` anywhere. Customizing ids is §3d.
...
Inside `shiny::moduleServer()`, pass `session$ns` — it is a namespace
function identical in shape to `shiny::NS(id)` for the enclosing
module id, so the UI (built with `shiny::NS(id)`) and the server
(using `session$ns`) agree. No double-prefixing occurs because
ggpaintr applies `ns` exactly once to raw ids.

```{r ns-module, eval = FALSE}
tab_ui <- function(id) {
  ns <- shiny::NS(id)
...
### 3e. Overriding UI copy {#3e-ui-text}
...
| Drop ggpaintr widgets into my own UI                                       | `ptr_input_ui()` + `ptr_output_ui()`                                       |
...
| Override widget labels / help text / placeholders                          | `ui_text = list(...)` on `ptr_server_state()` (see §3e)                    |
