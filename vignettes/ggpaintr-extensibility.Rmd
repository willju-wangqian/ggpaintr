...
- **`ui_text`** — override labels, help text, placeholder hints, and
  the fallback text for empty inputs. See §3d.
...
`ids = ...` anywhere. Customizing ids is §3c.
...
`ptr_server_state()` stores the ids on `ptr_state$ids`, so on every
other helper the `ids =` argument defaults to `ptr_state$ids`. Passing
it explicitly is still useful when you want the same state object to
drive more than one output location.
...
### 3d. Overriding UI copy {#3d-ui-text}
...
| Override widget labels / help text / placeholders                          | `ui_text = list(...)` on `ptr_server_state()` (see §3d)                    |
