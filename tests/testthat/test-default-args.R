...
test_that("ptr_default_symbol_or_string accepts a bareword symbol", {
  v <- ptr_default_symbol_or_string()
  expect_equal(v(quote(mpg)), "mpg")
})

test_that("ptr_default_symbol_or_string accepts a string literal verbatim", {
  v <- ptr_default_symbol_or_string()
  expect_equal(v("miles per gallon"), "miles per gallon")
})

test_that("ptr_default_symbol_or_string accepts the empty string (A3)", {
  v <- ptr_default_symbol_or_string()
  expect_equal(v(""), "")
})

test_that("ptr_default_symbol_or_string preserves backticked names (A4)", {
  v <- ptr_default_symbol_or_string()
  expr <- rlang::parse_expr("`miles per gallon`")
  expect_equal(v(expr), "miles per gallon")
})

test_that("ptr_default_symbol_or_string rejects numbers and calls", {
  v <- ptr_default_symbol_or_string()
...
test_that("ptr_default_string accepts a string literal", {
  v <- ptr_default_string()
  expect_equal(v("hi"), "hi")
})

test_that("ptr_default_string accepts the empty string (A3)", {
  v <- ptr_default_string()
  expect_equal(v(""), "")
})

test_that("ptr_default_string rejects a symbol or number", {
  v <- ptr_default_string()
...
test_that("ptr_default_numeric accepts numeric literals (incl. negative, sci)", {
  v <- ptr_default_numeric()
  expect_equal(v(5), 5)
  expect_equal(v(-5), -5)
  expect_equal(v(2.5e3), 2500)
})

test_that("ptr_default_numeric accepts unary minus on a literal AST", {
  v <- ptr_default_numeric()
  expect_equal(v(quote(-5)), -5)
})

test_that("ptr_default_numeric accepts `2 * pi` via baseenv parent", {
  v <- ptr_default_numeric()
  expect_equal(v(quote(2 * pi)), 2 * pi)
})

test_that("ptr_default_numeric rejects symbols not in allowlist", {
  v <- ptr_default_numeric()
  expect_error(
    v(quote(5L * x)),
...
test_that("ptr_default_numeric rejects unregistered functions", {
  v <- ptr_default_numeric()
  expect_error(
    v(quote(log10(100))),
...
test_that("ptr_default_numeric rejects multi-element vectors", {
  v <- ptr_default_numeric()
  expect_error(v(quote(c(1, 2))), class = "rlang_error")
})
...
test_that("ptr_default_numeric_vector accepts c(0, 1) and c(-1, 1) at length=2L", {
  v <- ptr_default_numeric_vector(length = 2L)
  expect_equal(v(quote(c(0, 1))), c(0, 1))
  expect_equal(v(quote(c(-1, 1))), c(-1, 1))
})

test_that("ptr_default_numeric_vector accepts seq() via constant-fold", {
  v <- ptr_default_numeric_vector()
  expect_equal(v(quote(seq(0, 1, by = 0.1))), seq(0, 1, by = 0.1))
})

test_that("ptr_default_numeric_vector enforces explicit length when given", {
  v2 <- ptr_default_numeric_vector(length = 2L)
  expect_error(v2(quote(c(0))), regexp = "length 2", class = "rlang_error")
})

test_that("ptr_default_numeric_vector rejects system() in any position", {
  v <- ptr_default_numeric_vector()
  expect_error(
...
test_that("ptr_default_numeric_vector validates the `length` argument", {
  expect_error(ptr_default_numeric_vector(length = "two"), class = "rlang_error")
  expect_error(ptr_default_numeric_vector(length = -1L), class = "rlang_error")
})

# ---- ptr_default_expression ------------------------------------------------

test_that("ptr_default_expression returns its input unchanged", {
  v <- ptr_default_expression()
  e <- quote(x + 1)
  expect_identical(v(e), e)
})

test_that("ptr_default_expression does not warn on bare expressions", {
  v <- ptr_default_expression()
  expect_no_warning(v(quote(x + 1)))
})

test_that("ptr_default_expression warns on quote() wrapper (A5.b)", {
  v <- ptr_default_expression()
  e <- quote(quote(x + 1))
...
test_that("ptr_default_expression warns on rlang::expr() wrapper (A5.b)", {
  v <- ptr_default_expression()
...
test_that("ptr_default_expression warns on bquote() and rlang::quo() wrappers", {
  v <- ptr_default_expression()
  expect_warning(v(quote(bquote(x + 1))), regexp = "already captures")
  expect_warning(v(quote(rlang::quo(x + 1))), regexp = "already captures")
})

test_that("ptr_default_expression stores system() verbatim (never evals)", {
  v <- ptr_default_expression()
...
test_that("ptr_register_constant_fold lets authors extend the allowlist", {
  ...
  expect_equal(ptr_default_numeric()(quote(log10(100))), 2)
})
...
test_that("validators never call eval() on rejecting input (system not run)", {
  ...
  expect_error(ptr_default_numeric()(quote(system("rm"))),
               regexp = "system.*not in constant-fold allowlist")
  expect_error(ptr_default_numeric_vector()(quote(system("rm"))),
               regexp = "system.*not in constant-fold allowlist")
})
