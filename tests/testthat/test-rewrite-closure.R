...
test_that("a function() literal in the formula does not crash translation", {
  expect_s3_class(
    ptr_translate(
      "tibble(k = 1:3) |>
         dplyr::mutate(m = purrr::map(k, function(.k) .k + num)) |>
...
test_that("a placeholder inside a closure body gets an id", {
  ...
  phs <- find_nodes(r, function(x) is_ptr_placeholder(x) && x$keyword == "num")
...
test_that("a closure-body placeholder appears in the runtime input spec", {
  ...
  num_ids <- spec$input_id[!is.na(spec$keyword) & spec$keyword == "num"]
...
test_that("a closure with a substituted placeholder evaluates end to end", {
  ...
  r <- ptr_translate(
    "mtcars |>
       tidyr::nest(.by = cyl) |>
       dplyr::mutate(fit = purrr::map(data, function(.d) lm(expr, data = .d))) |>
...
test_that("a literal NULL argument keeps its slot (labs(y = NULL))", {
  r <- ptr_translate(
    "ggplot(mtcars, aes(mpg, hp)) + geom_point() + labs(title = text, y = NULL)"
  )
