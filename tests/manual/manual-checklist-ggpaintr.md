...
3. With `ggplot` selected, confirm two sub-tabs appear (`Data`, `Controls`); Data holds the pipeline `num` control, Controls holds the `var` pickers. There is no per-layer Update Data button — pickers refresh lazily via the reactive cascade.
...
8. Re-launch the app with `ui_text = list(shell = list(layer_picker = list(label = "Choose layer"), data_subtab = list(label = "Pipeline"), controls_subtab = list(label = "Aesthetics")))` and confirm those three labels override defaults while everything else (Update plot, layer checkbox) keeps default copy.
...
3. On first paint (no inputs touched yet), confirm the `var` pickers in the Controls sub-tab are pre-populated with mtcars column names. Pre-warm runs every layer's pipeline once at server startup.
4. Set `filter(num) = 6`, `head(num) = 10`. Confirm the `var` pickers continue to show mtcars columns (the pipeline is reactive, but for these `num` producers downstream column names don't change). No click required.
5. Pick `x = mpg`, `y = disp`, click `Update plot`, and confirm the plot renders.
6. Change `head(num)` to `3` without clicking `Update plot`. Confirm `outputCode` and the rendered plot still reflect the last committed snapshot (`head(10)`) — Update Plot is the only commit trigger for the rendered output.
7. Click `Update plot`. Confirm `outputCode` now shows `head(3)` and the `var` pickers retain `mpg` / `disp` (selection preserved across cascade rebuilds).
8. Re-launch with two pipeline-bearing layers:
   ...
   Confirm each layer panel has its own Data sub-tab. Edit each layer's `num` independently and confirm the layers' consumers stay scoped — picking a column for `ggplot` does not affect `geom_point`'s pipeline or vice versa, since each layer's per-consumer reactive walks only its own `node$upstream`.
9. Re-launch with a nested `data = ...` chain:
    ...
    Confirm pipeline controls still surface in the Data sub-tab and that `outputCode` after clicking `Update plot` includes the full piped expression.
10. Launch a pipeline with a `text` placeholder, e.g. `iris |> dplyr::filter(Species != text) |> ggplot(aes(x = var, y = var)) + geom_point()`. Type `"setosa"` (with quotes), then click `Update plot`. Confirm the rendered code and plot use a single set of quotes (`Species != "setosa"`, not double-escaped).
11. **Intra-pipeline cascade.** Launch `mtcars |> dplyr::select(var) |> dplyr::select(var) |> ggplot(aes(x = var))`. The Data sub-tab has two `var` pickers (one per `select`). Pick a column in the first `var`; confirm the second `var`'s available choices update to that single column on the next Shiny flush, with no Update Plot click required.
