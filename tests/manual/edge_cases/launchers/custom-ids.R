...
# into ptr_module_ui()/ptr_module_server() and every internal id is prefixed.
...
ui <- fluidPage(
  ptr_module_ui(NS_PREFIX, "ggplot(mtcars, aes(x = mpg, y = hp)) + geom_point(size = num)")
)
server <- function(input, output, session) {
  ptr_module_server(NS_PREFIX, "ggplot(mtcars, aes(x = mpg, y = hp)) + geom_point(size = num)")
}
