...
paintr_components <- function(id, data_vars, expr) {
  code <- enexpr(expr)
  # code <- expr
  ...
  plot_settings <- if (has_name(gg_components, "plot_settings")) names(gg_components[['plot_settings']]) else NULL

  shiny_components <- controlUI(id, data_vars,
                                mapping, geom_args, plot_settings)
...
scaleColorWrapper <- function(id, paintr_obj, data, color_name, scaleColor_name) {
  moduleServer(
    ...
    function(input, output, session) {
      ...
      id_container <- reactive({
        req(paintr_obj())

        color_id <- paintr_get_ui(paintr_obj(), color_name, type = "id", scope = "mapping")
        scaleColor_id <- paintr_get_ui(paintr_obj(), scaleColor_name, type = "id", scope = "mapping")

        list(color_id = color_id, scaleColor_id = scaleColor_id)

      })

      selectedColors_box <- reactive({
        req(id_container(), data())
        ...
        bindCache(input[[id_container()[['color_id']]]]) %>%
        bindEvent(input[[id_container()[['color_id']]]], ignoreInit = TRUE)

      observe({
        req(selectedColors_box(), id_container())

        if(selectedColors_box()[['type']] == "TOO_MANY_LEVELS") {
          output[[id_container()[['scaleColor_id']]]] <- renderUI({
