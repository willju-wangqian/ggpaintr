...
`ggpaintr` works by treating a ggplot-like formula string as a template
with placeholders that become Shiny inputs.

-   `var` selects a variable or expression-like input
...
The low-level workflow is: parse a formula, build a runtime from user
inputs, then inspect the generated code and plot.
...
You can use the low-level runtime helpers directly when you want to
inspect or test the generated plot code outside Shiny.
...
Use `ggpaintr_app()` for the default wrapper when you want the package
to build the Shiny app shell for you.
...
The phase-1 extensibility layer lets you keep your own layout while
...
`vignette("ggpaintr-extensibility")` for custom Shiny integration
recipes. See `vignette("ggpaintr-placeholder-registry")` for the full
