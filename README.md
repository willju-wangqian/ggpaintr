...
`aes()`, and `geom_*()` calls work directly inside formula strings.
...
For a bslib-themed variant with the same API, use `ptr_app_bslib()`.
...
Call `ptr_app()` or `ptr_app_bslib()`. No Shiny code required. This is
the right entry point for teaching, demoing, or quick data exploration.
...
(custom placeholder types). `ptr_app_bslib()` adds `theme` and `title`
for the bslib shell.

Read `vignette("ggpaintr-workflow")` for the full list of arguments, the
three data-source paths (`data = <frame>`, the `upload` keyword, and
`ptr_normalize_column_names()` for non-syntactic local data), and a
walk-through of what happens when you click **Update plot**.
...
-   Non-module embedding: `ptr_input_ui()` and `ptr_output_ui()` drop
    the generated controls and output panes into any UI, while
    `ptr_server()` wires up the server side in one call.
-   Shiny module embedding: `ptr_module_ui()` and `ptr_module_server()`
...
-   For finer control, compose the individual `ptr_register_*()` helpers
    ...
    `ptr_setup_controls()` to observe the underlying reactive state.
-   `ptr_build_ids()` remains available for non-module and advanced
    custom layouts that need renamed raw top-level ids;
    `ptr_server_state()` exposes `raw_ids`, `ui_ids`, and `server_ids`
    for debugging namespace wiring.
-   `ui_text` rewrites labels, help text, and shell chrome at any of six
    sections (`shell`, `upload`, `layer_checkbox`, `defaults`, `params`,
    `layers`).

Read `vignette("ggpaintr-extensibility")` for runnable examples of both
Level 2 routes, the complete `ui_text` override catalog, and the
six-section merge precedence rules.
...
Read `vignette("ggpaintr-extensibility")` for runnable Level 3 recipes:
a headless plot rendered to a png file with no Shiny, a custom
`renderPlot()` that post-processes the ggplot object, a host app
injecting layers with `ptr_gg_extra()`, and programmatic code generation
with `ptr_extract_code()`.

### Customizing placeholders

Custom widget types — a date picker, a slider, a column-subset chooser —
are defined with `ptr_define_placeholder()` and mixed in with
`ptr_merge_placeholders()`. They plug into every level:
...
contract, a minimal end-to-end example, and the recipe for replacing a
built-in widget with a custom one.

## Where to go next

-   **`vignette("ggpaintr-workflow")`** — formula syntax, every
    placeholder keyword, every `ptr_app()` argument, data-source paths,
    and a conceptual tour of what happens between click and plot. Read
    this first.
...
-   **`vignette("ggpaintr-placeholder-registry")`** — the five
    placeholder hooks, required vs optional, worked examples of adding a
    new widget type, and the recipe for replacing a built-in
    placeholder.
...
-   Raw placeholder ids such as `"ggplot_3_2"` are not a stable
    ...
    Deeper traversal details (`index_path` encoding, internal companion
