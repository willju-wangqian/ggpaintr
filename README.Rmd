...
- Use `ptr_runtime_input_spec()`, `ptr_parse_formula()`,
  ...
  testing workflows.
- Use `ptr_generate_shiny()` to export an explicit standalone app script.
...
Use `ptr_generate_shiny()` when you want an explicit, editable Shiny app file that
...
`ptr_extract_plot()` inside your own `renderPlot()`.
...
placeholder hooks inline inside the `ptr_define_placeholder()` call. The export
...
- exported apps keep the current explicit-file shape built around
  `ptr_server()`
- only the six top-level ids exposed by `ptr_build_ids()` are configurable
