...
There is **no `ns =` argument** on Level-2 helpers. Namespacing happens through the **module `id`** you pass to `ptr_module_ui()` / `ptr_module_server()`. Each `id` is its own namespace; widget ids in different modules never collide.
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6, ptr_module_ui("plot_1", plots[[1]])),
    column(6, ptr_module_ui("plot_2", plots[[2]]))
  )
)
...
A shared key referenced in **exactly one** formula stays inline in *that* module's shared section; a key referenced in **two or more** goes to the standalone panel. Mix them:
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6, ptr_module_ui("plot_1", plots[[1]])),  # ax1 inline here
    column(6, ptr_module_ui("plot_2", plots[[2]]))   # ax2 inline here
  )
)
server <- function(input, output, session) {
  sh <- ptr_shared_server(obj)
...
`ptr_shared()` errors if no formula in `formulas` declares a `shared = "..."` annotation — drop the coordinator and use plain `ptr_module_ui()` / `ptr_module_server()` with distinct `id`s if nothing is shared. A **single** instance never builds the coordinator either: its shared keys are formula-local by definition and auto-render inline (that is L1 `ptr_app()` or one `ptr_module_*()` — see `level2_module`).
...
- **`shared = "..."` declared, no coordinator wired.** `ptr_module_server()` aborts with a clear "build the coordinator with `ptr_shared()` / `ptr_shared_server()`" message.
...
- **`shared_ui` builder produces the wrong widget kind.** The runtime validates the resolved value against each downstream placeholder type — e.g. a slider feeding a `var` consumer surfaces as an inline validation error.
