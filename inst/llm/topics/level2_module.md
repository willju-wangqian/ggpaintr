# Level 2 — the self-contained module pair
...
There is exactly one default-layout block at L2 — the module pair `ptr_module_ui()` / `ptr_module_server()`. It is **self-contained**: it owns its own `.ptr-app` theme scope and bundled asset dependency, so it drops straight into a host page with no scaffolding (this is the `ptr_<x>` half of the naming convention). There is **no** free-form controls-here / plot-there split at L2; placing controls and the plot in different regions, or replacing a built-in pane with your own renderer, is **L3** (see `level3_layout`, `level3_custom_render`).

## Module path — `ptr_module_ui()` + `ptr_module_server()`

One `id`, controls + outputs live next to each other inside the module's namespace.
...
`ptr_module_server()` returns the `ptr_state` from `ptr_init_state()`, so an embedder that wants to react to it (call `ptr_gg_extra()`, read `ptr_extract_*()`, render a second view) can capture it; otherwise the side effects are all that matter. `...` is forwarded to `ptr_init_state()` — so `ui_text =`, `checkbox_defaults =`, `expr_check =`, `safe_to_remove =`, and the shared-binding arguments are all accepted here too.

## Shared widgets: single instance vs. multiple instances

A `keyword(shared = "<key>")` placeholder is driven by one widget in lockstep wherever the key appears. *How* it surfaces is decided **per key by how many formulas reference it** — the partition rule:
