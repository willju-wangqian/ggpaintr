...
# data <- iris
# data <- faithfuld
# data <- data.frame(trt=LETTERS[1:5], l=c(20, 40, 10, 30, 50), r=c(70, 50, 30, 60, 80))
...
server <- function(input, output) {
  ...
  data_container <- reactive({
    ...
    # data.frame(trt=LETTERS[1:5], l=c(20, 40, 10, 30, 50), r=c(70, 50, 30, 60, 80))
  })
  ...
  box_main <- reactive({
    ...
    paintr(
      box_control_id,
      data, data_path = input$defaultData,
      # geom_point(aes(x, y, color)) +
      #     theme_choose +
      #     labs(x, y, title)
      geom_point(aes(x, y, color), alpha) +
      # geom_lollipop(aes(x, y)) +
      # geom_dumbbell(aes(x, y, xend)) +
        theme_choose +
        labs(x, y, title)
