...
*ADR status: "proposed — implementation pending" · Date: 2026-05-20 · **Code state: MERGED 2026-05-21** (per auto-memory `project-adr9-merged`; commit `2ee656b`).* The big placeholder-grammar reform. Three things: (1) renamed all keywords with a `pp` prefix — `var`→`ppVar`, `text`→`ppText`, `num`→`ppNum`, `expr`→`ppExpr`, `upload`→`ppUpload`. **The old un-prefixed keywords no longer parse.** (2) `ppExpr` now accepts a user-typed expression at runtime (the "expression-mode" input — closes the "user can type their own ggplot fragment" feature). (3) Registry entries declare their argument schema so the constructors can validate at registration time. Closely paired with ADR 0010.
