...
- `ptr_exec()` orchestrates: complete expressions → evaluate layers → build plot → validate. Returns `list(ok, plot, code_text, message, condition)`.
...
- Public API: functions prefixed `ptr_` or `ptr_`. Internal helpers are unexported.
