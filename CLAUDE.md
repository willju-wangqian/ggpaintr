...
`paintr_formula()` parses the formula via rlang, splits it into layers (e.g. `ggplot()`, `geom_point()`), identifies placeholder tokens in arguments, generates unique IDs like `"ggplot+3+2"`, and returns a `paintr_obj` containing ASTs, placeholder metadata, and the placeholder registry.
...
- UI generation: `paintr_build_ui_list()` creates Shiny controls; `var` inputs use deferred `uiOutput()` since columns depend on data.
- Expression completion: `paintr_complete_expr()` substitutes user inputs into layer ASTs.
- `paintr_build_runtime()` orchestrates: complete expressions → evaluate layers → build plot → validate. Returns `list(ok, plot, code_text, message, condition)`.
...
- Public API: functions prefixed `ggpaintr_` or `paintr_`. Internal helpers are unexported.
