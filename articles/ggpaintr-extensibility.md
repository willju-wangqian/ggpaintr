...
- **`ui_text`** — override labels, help text, placeholder hints, and the
  fallback text for empty inputs. See §3f.
...
server id named in `ptr_state$server_ids` (`ptr_state$ids` remains a
compatibility alias). The default ids above match the defaults from
...
### 3c. Shiny module integration

If you prefer Shiny modules, or already build apps with module habits,
use
...
compact template for the ID contract Shiny modules need when UI is
generated from
[`renderUI()`](https://rdrr.io/pkg/shiny/man/renderUI.html): raw ids
stay local to the module server, and DOM ids are rendered through the
module namespace.
...
The single failure mode for non-module multi-instance apps is reusing
the same `ns` (or leaving it at its default) across two instances in the
same root session. Then the server ids collide and Shiny silently keeps
only the last `output[[id]] <-` assignment — one instance appears empty.
Sanity check any wiring by inspecting the explicit ID contract in the
console:
...
length(intersect(unlist(state_a$ui_ids), unlist(state_b$ui_ids)))  # expect 0
length(intersect(unlist(state_a$server_ids), unlist(state_b$server_ids)))  # expect 0
```
...
[`ptr_server_state()`](https://willju-wangqian.github.io/ggpaintr/reference/ptr_server_state.md)
stores the raw top-level ids on `ptr_state$raw_ids`, rendered DOM ids on
`ptr_state$ui_ids`, and Shiny binding keys on `ptr_state$server_ids`.
`ptr_state$ids` remains as a compatibility alias for
`ptr_state$server_ids`. Every helper reads the state contract
automatically — so you only pass `ids` once, at state construction time.

Only these five raw top-level ids are configurable. Per-placeholder
input ids (the ones generated by
...
ggpaintr namespaces them at the UI/server boundary instead of rewriting
the parsed formula state.

### 3f. Overriding UI copy
...
#### 3f.2 Top-level sections and merge precedence
...
#### 3f.3 Chrome sections — `shell`, `upload`, `layer_checkbox`
...
#### 3f.4 Worked example — same placeholder, three scopes
...
#### 3f.6 Validating a `ui_text` list before use
...
#### 3f.7 Full worked embed with copy overrides
